...
void	*inextztm(tsdn_t *tsdn, size_t size, szind_t ind, bool zero,
    tcache_t *tcache, bool is_metadata, arena_t *arena, bool slow_path);
void	*ialloc(tsd_t *tsd, size_t size, szind_t ind, bool zero,
    bool slow_path);
void	*inext(tsd_t *tsd, size_t size, szind_t ind, bool zero,
    bool slow_path);
...
JEMALLOC_ALWAYS_INLINE void *
inext(tsd_t *tsd, size_t size, szind_t ind, bool zero, bool slow_path)
{

	return (inextztm(tsd_tsdn(tsd), size, ind, zero, tcache_get(tsd, true),
	    false, NULL, slow_path));
}
